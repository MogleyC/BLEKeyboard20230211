...
void services_init()
{
	// Queued Writes module을 초기화 
	// 실질적으로는 여기서 BLE에 등록되지 않음
	// 여러개가 생성 가능한 것으로 보임
    qwr_init();
	// 디바이스 인포메이션 서비스
	// 여러개 생성 불가
    ...
    bas_init();
	// HID 서비스
	// NRF_SDH_BLE_TOTAL_LINK_COUNT 를 수정 -> X
	// 여러개의 HID 서비스를 만들어야 한다.
	// 이것을 여러번 호출하면 각기 다른 UUID의 HID가 생성되는 것 같다.
    hids_init();
}
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
   ...
   //         APP_ERROR_CHECK(err_code);
			...
			////BLE Queued Writes module에 연결 핸들을 등록한다.
   ...
   //         // Send next key event
			//// 예약된 키를 발송한다. 이 예약은 한번에 많은 키를 발송할 때 HID한계에 의해 미처 다 발송하지 못할때 발생한다.
...
{
	//// HiD이벤트가 발생될때 호출된다.
	//// ble_hids의 p_hids->evt_handler에 연결되어 여러 부분에서 호출된다.
